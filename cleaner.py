...
class DataCleaner:
    ...
    def remove_na_adjust_weights(self,
        ...
        if verbose:
            ...
            if by_vars:
                missing_in_groupvars = result[by_vars].isna().any(axis=1).sum()
                if missing_in_groupvars > 0:
                    warnings.warn(
                        f"Found {missing_in_groupvars} rows with missing values in grouping variables {by_vars}. "
                        f"These will be adjusted as a separate group to maintain total weights."
                    )
    ...
    def _adjust_weights_by_group(self, 
        ...
        if not by_vars:
            ...
            # IMPORTANT: groupby() excludes rows with NaN in grouping variables
            # We need to handle these separately to avoid weight leakage
            
            # First, handle rows with non-missing grouping variables
            groupby_mask = ~data[by_vars].isna().any(axis=1)
            
            for group_vals, group_data in data[groupby_mask].groupby(by_vars, dropna=False):
            ...
            # Second, handle rows with ANY missing values in grouping variables
            # These are adjusted globally (as a separate group)
            missing_groupvar_mask = ~groupby_mask
            if missing_groupvar_mask.any():
                missing_keep_mask = missing_groupvar_mask & keep_mask
                
                original_sum = data.loc[missing_groupvar_mask, weight_col].sum()
                remaining_sum = data.loc[missing_keep_mask, weight_col].sum()
                
                if remaining_sum > 0:
                    adjustment_factor = original_sum / remaining_sum
                    adjusted_weights[missing_keep_mask] *= adjustment_factor
